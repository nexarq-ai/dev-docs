...
-   **Kernel Space**: Privileged mode where the kernel executes. Manages hardware resources (CPU, memory, I/O) and provides system calls. [The Linux Kernel Documentation](https://www.kernel.org/doc/html/latest/).
...
### Process Management
-   **PID (Process ID)**: Unique identifier for every running process. [Process Management in Linux (Red Hat)](https://www.redhat.com/sysadmin/linux-process-management).
...
-   **File Descriptors (FD)**: Integers representing open files/sockets. High-concurrency servers (like Nginx) require high `ulimit -n` values to handle thousands of connections. [What are File Descriptors? (DigitalOcean)](https://www.digitalocean.com/community/tutorials/understanding-file-descriptors-in-linux).
